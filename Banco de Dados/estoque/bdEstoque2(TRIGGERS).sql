...
VALUES (1,1,'200','1500.00')
   ...
   /* Criar um trigger que, ao ser feita uma venda (Insert na tabela tbItensVenda),todos os produtos vendidos tenham sua quantidade atualizada
   na tabela tbProduto. Exemplo, se foi feita uma venda de 5 unidades do produto código 01,
   na tabela tbProduto a quantidade desse produto será a quantidade atual –5; */

   CREATE TRIGGER tgReduzProduto
   ON tbItensVenda
   AFTER INSERT

   AS

   DECLARE @produto VARCHAR, @quantidadeItensVenda INT
   ...
   SET @quantidadeItensVenda = (SELECT quantidadeItensVenda FROM INSERTED)
	...
	/* Criar uma trigger que, quando for inserida uma nova entrada de produtos na tbEntradaProduto, a quantidade desse 
	produtos e ja atualizada e aumenta da na tabela tbProduto; */

	CREATE TRIGGER tgEntradaProdutos
	ON tbEntradaProduto
	AFTER INSERT

	AS

	DECLARE @produto VARCHAR, @quantidadeEntradaProduto INT
   ...
   SET @quantidadeEntradaProduto = (SELECT quantidadeEntradaProduto FROM INSERTED)
	...
	/* Criar uma trigger que, quando for feita uma venda de um determinado produto, se ja feito um Insert na tbSaidaProduto. */
   ...
   SET @quantidadeItensVenda = (SELECT quantidadeItensVenda FROM INSERTED)
	...
	VALUES
	(GETDATE(), @produto, @quantidadeItensVenda)
